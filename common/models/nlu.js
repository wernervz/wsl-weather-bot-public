...
var apiUtils = require('../util/api')

module.exports = function (NaturalLanguage) {
  NaturalLanguage.disableRemoteMethodByName('invoke', true)
  // Define the getSentiment method
  NaturalLanguage.getSentiment = async function (text, cb) {
    let response = {}
    try {
      response = await apiUtils.genericRequestRawPromise({
    ...
    if (response && response.sentiment && response.sentiment.document) {
